...
- **Confidently wrong — 推測當結論**：Zoe 會高信心地聲稱根因但沒有驗偽。例如「cosine threshold 有效，從 10 降到 2」但沒驗證 PG 是否被呼叫（實際走 Qdrant）。**所有推測必須標為假說 + 列驗偽計畫，驗偽後才能升級為結論。** 不要讓 CEO 猜你是不是在 confidently wrong。
...
2. docs/decisions.md           — 商業 + 技術決策（全部讀完）
...
- **推測 ≠ 結論**。所有技術推測標記為「假說」，不用肯定語氣
- **列驗偽計畫**：「如果正確，應觀察到 X；如果錯誤，應觀察到 Y」
- **驗偽後才升級為結論**。最少要告訴 CEO 驗偽計畫
- 例：不說「cosine threshold 有效」，說「假說：threshold 生效。驗偽：查 server log 確認走 PG 而非 Qdrant」
