...
> **Cloud Embedding（2026-03）**：全量 indexing 改由 GCP L4 VM 執行。`cloud_embed.py` 只做 TSV → chunking → Qwen3-4B embedding → .jsonl+.npy 輸出（不碰 DB）。結果下載後由獨立 bulk load 腳本灌入 PG。Sub-batch limit 30K chunks/call 避免 VRAM OOM。
