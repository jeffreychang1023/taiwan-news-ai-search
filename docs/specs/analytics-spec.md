...
**`get_query_stats(days=7)`**：讀取近 N 天統計，回傳 total_queries, avg_latency_ms, total_cost_usd, error_rate, click_through_rate。
...
| `llm_relevance_score` | REAL | DOUBLE PRECISION | LLM 整體相關度 |
| `llm_keyword_score` | REAL | DOUBLE PRECISION | LLM 關鍵字匹配分數 |
| `llm_semantic_score` | REAL | DOUBLE PRECISION | LLM 語意相關度 |
| `llm_freshness_score` | REAL | DOUBLE PRECISION | LLM 新鮮度分數 |
| `llm_authority_score` | REAL | DOUBLE PRECISION | LLM 權威性分數 |
| `llm_final_score` | REAL | DOUBLE PRECISION | LLM 綜合分數 |
