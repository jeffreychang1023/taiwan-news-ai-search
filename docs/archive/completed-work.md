...
## ✅ Track AI：Prompt 全面繁中改造 + Dead Code 歸檔（2026-03-23）

**目標**：將系統所有 LLM prompt 從英文改造為繁體中文，移除 dead code，建立 prompt 維護 skill。

### 改造範圍（5 個 Batch）

| Batch | 項目 | 檔案 |
|-------|------|------|
| B1 | SummarizeResultsPrompt、SynthesizePromptForGenerate | `config/prompts.xml` |
| B2 | RankingPrompt、RankingPromptForGenerate、DescriptionPromptForGenerate | `config/prompts.xml` |
| B3 | DetectIrrelevantQuery、PrevQueryDecontextualizer、DecontextualizeContext、FullDecontextualize、DetectMemoryRequest、QueryRewrite、DetectMultiItemType、DetectItemType、DetectQueryType、DiversityReranking | `config/prompts.xml` |
| B4 | Free Conversation 3 變體、Time Range Extractor、Ranking fallback | `methods/generate_answer.py`、`core/query_analysis/time_range_extractor.py`、`core/ranking.py` |
| B5（精簡） | QueryUnderstanding 移除英文括號標籤、Reasoning agents 移除「一個」等冗詞 | `reasoning/prompts/*.py` |

### Dead Code 歸檔

- 18 個 dead code prompt 從 `config/prompts.xml` 移除
- 歸檔至 `legacy/prompts/archived-prompts.xml`（含原始行號、用途、被取代原因）
- 移除整個 Recipe section 和 reasoning placeholder section

### 改造原則（已記錄至 skill）

1. 全面繁中，JSON key 等程式碼層不動
2. 「一律使用繁體中文回應，除非使用者明確要求用英文」（英文 only 例外，不支援其他語言）
3. 專業記者/研究員口吻
4. 精簡冗詞：的、也、將、就、在、一、便、了
5. 用【】取代 CRITICAL/IMPORTANT
6. 不動程式碼層語法（跳脫符號、template 變數、XML 標籤）
7. 來源權威性：專業匹配 100 / 政府 95 / 一般 80（所有收錄來源皆可信）

### 新建檔案

- `.claude/commands/prompt-rewrite.md` — prompt 改造原則與進度 skill
- `legacy/prompts/archived-prompts.xml` — dead code 歸檔

### 修改的檔案

`config/prompts.xml`、`code/python/methods/generate_answer.py`、`code/python/core/query_analysis/time_range_extractor.py`、`code/python/core/ranking.py`、`code/python/reasoning/prompts/analyst.py`、`code/python/reasoning/prompts/critic.py`、`code/python/reasoning/prompts/writer.py`、`code/python/reasoning/prompts/clarification.py`、`code/python/reasoning/prompts/cov.py`
...
## ✅ Session 切換穩定性（2026-03-13）

**問題**：搜尋中切換 session → 原 session 結果空白。
**最終方案**：Cancel + Retry Button — 切 session 時直接 cancel stream，標記 `interruptedSearch`，切回顯示 retry 按鈕。
**歷程**：auto re-search（無限迴圈）→ 背景 stream 繼續（stale reference + 跨 session 污染）→ single-stream-per-mode 限制（還是卡住）→ 全部拆掉改 cancel + retry。
