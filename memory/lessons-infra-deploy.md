...
**檔案**：VPS `/root/nlweb/docker-compose.yml`
**日期**：2026-03-05
...
**解決方案**：手動加規則：`iptables -t nat -A POSTROUTING -s 172.18.0.0/16 ! -o br-575dcccc6b8c -j MASQUERADE`（infra_default 網路）。**待做**：持久化規則（`iptables-save` 或 `iptables-persistent` 套件），否則 VPS 重啟後規則又會丟失。
...
### GCP GPU quota 有兩層 — 全域 + 區域都要通過
**問題**：CLI `gcloud compute regions describe` 顯示 `NVIDIA_L4_GPUS: limit=1`（區域 quota），以為可以直接建 VM。實際建 VM 時被 `GPUS_ALL_REGIONS: limit=0`（全域 quota）擋住。兩層 quota 都必須 >= 1 才能開 GPU VM。Web Console 的配額頁面預設不顯示 GPU 項目，需用 "NVIDIA" 搜尋。
**解決方案**：在 IAM → 配額頁面搜尋 `GPUs (all regions)` → 編輯配額 → 申請 1 → 理由寫 `ML inference workload for text embedding`。審批通常幾分鐘到幾小時。
**信心**：高（實際踩坑，Free Trial 帳號驗證）
**檔案**：N/A（GCP 帳號層）
**日期**：2026-03-24

### GCP Spot VM 庫存極不穩定 — 建好後秒被回收
**問題**：L4 Spot VM 在 us-central1 全部 zone stockout，europe-west1-b 建成但狀態直接 STOPPING → TERMINATED（被 preempt）。Spot 的便宜代價是隨時可能被回收。
**解決方案**：改用 on-demand VM（~$1.1/hr vs Spot ~$0.4/hr）。Free Trial credits 充足時，穩定性比省錢重要。掃多個 zone 直到有庫存。
**信心**：高
**日期**：2026-03-24

### GCP VM scope ≠ IAM 權限 — 兩者都要設
**問題**：VM 建立時設了 `--scopes=storage-ro`，但 Compute Engine default SA 沒有 Storage IAM 角色。VM 上跑 gsutil 時 403 AccessDenied。
**解決方案**：bucket 層級加 IAM：`gsutil iam ch serviceAccount:{SA}:objectViewer gs://{bucket}`。scope 控制 VM 能請求的 OAuth 範圍，IAM 控制 SA 實際有的權限，兩者缺一不可。
**信心**：高
**日期**：2026-03-24

### Qwen3-4B INT8 在 L4 上實測 ~100 texts/s — 比直覺估計慢很多
**問題**：初始估算 4-5 小時完成全量 embedding（假設 ~500 chunks/s）。實測 L4 上 Qwen3-4B INT8 約 ~100 texts/s raw，有效速率（含 I/O、chunking）~18 chunks/s。4B 參數量 embedding model 不能用一般小模型（0.3-1B）的速度去估。
**教訓**：Embedding 速度估算公式應基於模型參數量。4B 模型在 L4 上的真實吞吐量約為 0.6B 模型的 1/5-1/6。
**信心**：高（11 小時實測數據）
**日期**：2026-03-24

### 大檔案一次性 embed 會 VRAM OOM — 需要 sub-batching
**問題**：`model.encode(texts, batch_size=32)` 中 batch_size 控制每次 forward pass 的量，但 sentence_transformers 內部會累積所有結果在 GPU memory。12 萬+ chunks 一次呼叫 → CUDA OOM（L4 23GB VRAM）。
**解決方案**：加 `EMBED_CHUNK_LIMIT = 30000`，超過則分 sub-batch 呼叫 `model.encode()`，每次之間 `torch.cuda.empty_cache()`。
**信心**：高（OOM 修復後驗證通過）
**檔案**：`code/python/indexing/cloud_embed.py`
**日期**：2026-03-24

### GCP Deep Learning VM 用 python3 不是 python — nohup 指令要注意
**問題**：`nohup python /tmp/cloud_embed.py` 靜默失敗（`python` 不存在），log 只顯示 `nohup: failed to run command 'python': No such file or directory`。Deep Learning VM image 只有 `python3`。
**解決方案**：用 `python3` 明確指定。通則：GCP DL VM、Ubuntu 22.04+ 預設都沒有 `python` alias。
**信心**：高
**日期**：2026-03-24

### VM 重啟後 /tmp 會被清空 — 上傳的腳本和資料會消失
**問題**：停止 VM 改 scope 後重啟，先前 scp 上傳到 `/tmp/` 的腳本消失。
**解決方案**：重啟後需重新上傳，或把檔案放在 `/home/` 而非 `/tmp/`。
**信心**：高
**日期**：2026-03-24

### gsutil wildcard 在 Windows shell 會被本地展開 — 檔案太多超出 argument limit
**問題**：`gsutil -m cp C:/path/*.tsv gs://bucket/` 在 Windows Git Bash 中，475 個檔名被本地 shell 展開，超出 argument list 長度限制。
**解決方案**：分批上傳（按 source prefix 拆分）或用引號包住 pattern 讓 gsutil 自己處理。
**信心**：高
**日期**：2026-03-24
...
**正確做法（已驗證）**：
1. **cloud-init user data**（`infra/cloud-init.yaml`）：SSH port 2222 + 禁密碼 + UFW 防火牆 + fail2ban
2. **Hetzner Cloud Firewall**（網路層）：只開 2222/80/443，在封包到達 VPS 前就擋掉。比 host-level UFW 更安全
