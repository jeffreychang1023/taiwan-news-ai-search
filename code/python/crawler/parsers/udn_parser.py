...
class UdnParser(BaseParser):
    ...
    # 使用 settings 中的常數
    DEFAULT_CATEGORY = settings.UDN_DEFAULT_CATEGORY
    COMMON_CATEGORIES = settings.UDN_COMMON_CATEGORIES
    ...
    async def get_latest_id(self, session=None) -> Optional[int]:
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as sess:
                async with sess.get(list_url, timeout=aiohttp.ClientTimeout(total=10)) as response:
    ...
    def _parse_date_string(self, time_str: str) -> Optional[datetime]:
        """解析 UDN 的時間字串（委託給 TextProcessor）"""
